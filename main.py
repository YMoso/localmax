...
def find_peaks_for_column(values, group_size, column_name):
    ...
    for i in range(1, len(values) - 1):
        ...
        if curr_val > prev_val and curr_val > next_val:
            ...
            peaks.append({"column": column_name,
...
def find_all_peaks(matrix):
    ...
    for col in matrix.columns:
        ...
        if len(peaks) > 0:
            best_peak = max(peaks, key=lambda p: (p["strength"], p["value"], p["group_size"], -p["matrix_index"]))
            best_in_columns[col] = best_peak
            all_peaks.extend(peaks)

    if len(all_peaks) == 0:
        return None, best_in_columns
    best_overall = max(all_peaks, key=lambda p: (p["strength"], p["value"], p["group_size"], -p["matrix_index"]))
...
def highlight_table(matrix, best_peak, best_by_col):
    ...
    for col, peak in best_by_col.items():
        ...
        style.loc[row, col] = "background-color: #ffdd57; color: black; font-weight: bold;"
    if best_peak is not None:
        row = best_peak["matrix_index"]
        col = best_peak["column"]
        style.loc[row, col] = "background-color: #8fd19e; color: black; font-weight: bold;"
    return style
...
default_df = pd.DataFrame({"number": [1, 2, 1, 2, 2, 3, 4, 2, 1, 2]})
